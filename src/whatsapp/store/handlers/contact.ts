	...
	const set: BaileysEventHandler<"messaging-history.set"> = async ({ contacts }) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during contacts set: ${typeof e === "object" && e && "message" in e ? (e as any).message : String(e)}`,
			);
		}
	};
	...
	const upsert: BaileysEventHandler<"contacts.upsert"> = async (contacts) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An unexpected error occurred during contacts upsert: ${typeof error === "object" && error && "message" in error ? (error as any).message : String(error)}`,
			);
		}
	};
		...
		for (const update of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during contact update: ${typeof e === "object" && e && "message" in e ? (e as any).message : String(e)}`,
				);
			}
		}
	};
