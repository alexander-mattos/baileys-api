	...
	const upsert: BaileysEventHandler<"groups.upsert"> = async (groups) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during groups upsert: ${typeof e === "object" && e && "message" in e ? (e as any).message : String(e)}`,
			);
		}
	};
		...
		for (const update of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during group metadata update: ${typeof e === "object" && e && "message" in e ? (e as any).message : String(e)}`,
				);
			}
		}
	};
	...
	const updateParticipant: BaileysEventHandler<"group-participants.update"> = async ({
		...
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during group participants update: ${typeof e === "object" && e && "message" in e ? (e as any).message : String(e)}`,
			);
		}
	};
